...
export function ApiKeysTable() {
  ...
  const { table } = useDataTable({
    ...
    globalFilter,
    pagination,
    // ===== CUSTOM: backend handles search (`/api/token/search?q=...&token=...`).
    // globalFilterFn returns true so react-table skips client-side filtering —
    // server rows have masked keys (e.g. "xxxx**********yyyy") that would be
    // wrongly dropped by .includes() against the raw user input. =====
    globalFilterFn: () => true,
    onPaginationChange,
    onGlobalFilterChange,
    onColumnFiltersChange,
