...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	defer func() {
		if newAPIError != nil {
			...
			if retryAfterSeconds := types.GetRetryAfterSeconds(newAPIError); retryAfterSeconds > 0 {
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	service.AnnotateDeepSeekFreeTierLimit(channelError.ChannelType, err)
	logger.LogError(c, fmt.Sprintf("channel error (channel #%d, status code: %d): %s", channelError.ChannelId, err.StatusCode, common.LocalLogPreview(err.Error())))
	// 不要使用context获取渠道信息，异步处理时可能会出现渠道信息不一致的情况
	// do not use context to get channel info, there may be inconsistent channel info when processing asynchronously
	if service.ShouldDisableChannel(err) && channelError.AutoBan {
		if types.IsChannelAutoDisableError(err) {
			service.DisableChannelUntil(channelError, err.ErrorWithStatusCode(), types.GetChannelAutoDisableUntil(err))
		} else {
			gopool.Go(func() {
